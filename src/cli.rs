...
pub fn handle_cli(args: &[String]) -> Result<()> {
    if args.len() == 1 {
        ...
        println!("🐾 Starting Mochi's terminal...");
...
fn show_help() -> Result<()> {
    println!("🐈 Digital Pet - Mochi");
    println!();
    println!("Usage:");
    println!("  pet               Open interactive terminal UI");
    println!("  pet status        Show pet status");
    println!("  pet feed          Feed Mochi");
    println!("  pet play          Play with Mochi");
...
fn cmd_feed() -> Result<()> {
    println!("🍖 Feeding Mochi... (coming after Phase 2)");
    Ok(())
}

fn cmd_play() -> Result<()> {
    println!("🎮 Playing with Mochi... (coming after Phase 2)");
    Ok(())
}

fn cmd_reset() -> Result<()> {
    println!("⚠️  Reset Mochi? This cannot be undone. (y/n)");
